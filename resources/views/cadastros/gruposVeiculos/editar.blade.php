...
@section('title')
Edição de grupos de veículos @parent
@stop
@section('content')
    ...
    <div class="container">
        <div class="page-title">
            <h2>
                <span class="flaticon-icon014"></span> Edição de grupo de veículos
            </h2>
        </div>
        <div id="formCadastro" class="panel panel-default">
            <form id="formEdicaoGrupoVeiculo" method="POST" action="{{ url('painel/cadastros/gruposVeiculos/editar/'.$grupo->gvcodigo) }}" class="form-horizontal" enctype="multipart/form-data">
                {{ csrf_field() }}
                    <div class="col-sm-6 form-group">
                        <div class="row">
                            <div class="col-sm-12 {{ ($errors->has('gvdescricao')) ? 'has-error' : '' }}">
                                <label>Grupo de Veículos*</label>
                                <input type="text" placeholder="Digite o nome do grupo" name="gvdescricao" value="{{ old('descricao') != null ? old('descricao'): $grupo->gvdescricao != null? $grupo->gvdescricao : ''}}" class="form-control">
                            ...
                            <div class="col-sm-12">
                                ...
                                <select name="gvempresa" value="{{ old('$grupo->gvempresa') }}" class="form-control desabilitar select2-noClear empresa-gv">
                                    @foreach ($clientes as $key => $cliente)
                                        ...
                                        @else
                                            <option {{ $grupo->gvempresa == $cliente->clcodigo ? 'selected' : '' }} value="{{ $cliente->clcodigo }}">{{ $cliente->clnome }}</option>
                                        @endif
                                    @endforeach;
                                </select>
                            </div>
                    ...
                    <input id="gvcodigo" type="hidden"  name="gvcodigo" value="{{ $grupo->gvcodigo }}">
                            ...
                            <div class="col-sm-12 {{ ($errors->has('veiculos')) ? 'has-error' : '' }}">
                                ...
                                <select multiple name="veiculos[]" value="" class="form-control desabilitar veiculos-grupo-veiculos select-selecionar-todos">
                                    @foreach ($veiculos as $key => $veiculo)
                                        ...
                                        @elseif($grupo->veiculos != null)
                                            <option {{ in_array($veiculo->vecodigo, $grupo->codigosVeiculos()) ? 'selected' : '' }} value="{{ $veiculo->vecodigo }}"> {{ $veiculo->veplaca.' | '.$veiculo->vedescricao }} </option>
                                        @else
